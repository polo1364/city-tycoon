// 城市資本戰 Web 版 v2.2：快取保險 + safeEnsureCash
"use strict";
(function(){
  ...
  function main(){
    ...
    async function onLandUtility(p,t,diceTotal){
      ...
      if (ownerIdx<0){
        var price=150;
        if (!p.isAI){ document.getElementById('turnInfo').textContent='可用 $'+price+' 購買《'+t.name+'》'; state.awaitingBuy=true; document.getElementById('btnBuy').disabled=!(p.cash>=price); document.getElementById('btnSkip').disabled=false; }
        else { if (aiShouldBuyUtility(p)) buyProperty(p, idx, price); }
      } else if (ownerIdx===state.cur){
        log(p.name+' 來到自己的《'+t.name+'》。', !p.isAI);
      } else {
        var owner=state.players[ownerIdx]; if (owner.mortgaged.indexOf(idx)>=0){ log('《'+t.name+'》已抵押，免租。', !p.isAI); return; }
    ...
    // 原生 ensureCash（已定義）；同時提供安全包裝避免舊版殘留
    ...
    async function safeEnsureCash(p, need, creditor){
      if (typeof ensureCash === "function") { return await ensureCash(p, need, creditor); }
      // Fallback（理論上不會觸發）：最小版只把所有資產抵押；仍不足則淘汰。
      var all = p.properties.slice();
      for (var i=0;i<all.length;i++){ doMortgage(p, all[i]); }
      if (p.cash < need){ p.alive=false; p.properties=[]; p.mortgaged=[]; renderPlayers(); updateOwners(); }
    }

    async function payToBank(p, amount, reason){ if (amount<=0) return; log('🏦 '+p.name+' 支付銀行 $'+amount+'（'+reason+'）', !p.isAI); await safeEnsureCash(p, amount); if (!p.alive) return; p.cash-=amount; renderPlayers(); }
    async function payToPlayer(payer,receiver,amount,reason){
      ...
      await safeEnsureCash(payer, amount, receiver); if (!payer.alive) return; payer.cash-=amount; receiver.cash+=amount; renderPlayers();
    }
    ...
    async function aiTurn(){
      ...
      if (p.inJail){
        if (p.jailTurns<2){
          ...
          await safeEnsureCash(p, JAIL_FINE);
